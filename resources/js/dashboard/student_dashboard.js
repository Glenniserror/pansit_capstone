...
function animateProgressBars() {
    ...
    progressBars.forEach((bar, index) => {
        ...
        setTimeout(() => {
            bar.style.transition = 'width 1.5s ease-out'; // Smoother, longer animation
            bar.style.width = targetWidth;
        }, index * 200); // Staggered delay for visual appeal
    });
}
...
function fadeInCards() {
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach((entry, index) => {
            if (entry.isIntersecting) {
                ...
                }, index * 150); // Shorter stagger for professional feel
            }
        });
    }, { threshold: 0.1 });
    ...
    cards.forEach(card => {
        ...
        card.style.transition = 'opacity 0.8s ease-out, transform 0.8s ease-out'; // Smoother transitions
        observer.observe(card);
    });
}

// Enhanced hover effects with subtle lifts and focus states
function addHoverEffects() {
    ...
    buttons.forEach(btn => {
        btn.addEventListener('mouseenter', () => {
            btn.style.transform = 'scale(1.02) translateY(-1px)';
            btn.style.boxShadow = '0 4px 12px rgba(0, 0, 0, 0.15)'; // Subtle glow
        });
        ...
        btn.addEventListener('focus', () => {
            btn.style.boxShadow = '0 0 0 2px rgba(37, 99, 235, 0.5)';
        });
    ...
    cards.forEach(card => {
        card.addEventListener('mouseenter', () => {
            card.style.transform = 'translateY(-4px)'; // Subtle lift
            card.style.boxShadow = '0 6px 20px rgba(0, 0, 0, 0.1)'; // Enhanced shadow
        });
